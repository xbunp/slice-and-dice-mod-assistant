...
public class FullScreenUIGenerator : MonoBehaviour
{
    ...
    private GridReferences BuildGrid(RectTransform panel, List<GridRowSpec> rows)
    {
        ...
        foreach (var rowSpec in rows)
        {
            ...
            // Math adjustment to span multiple row heights if requested
            float activeHeight = rowSpec.customHeight > 0 ? rowSpec.customHeight : 
            ...
            foreach (var cell in rowSpec.cells)
            {
                ...
                if (cellObj != null)
                {
                    ...
                    // Image panels act as background containers, so we remove cell-level margins
            ...
            // Do not decrement currentY if this row acts as a background container
    ...
    */
    ...
    private GridReferences BuildGrid(RectTransform panel, List<GridRowSpec> rows)
    {
        ...
        foreach (var rowSpec in rows)
        {
            ...
            RectTransform rowRt = CreateAndSetupRow(panel, currentY, activeHeight);
