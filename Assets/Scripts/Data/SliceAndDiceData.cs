...
public static class BaseItemMetadataRegistry
{
    ...
    // Normalized lookup set (lowercase, no spaces) generated automatically on startup
    private static readonly HashSet<string> NormalizedDiceAffectingItems = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

    static BaseItemMetadataRegistry()
    {
        foreach (var item in RawDiceAffectingItems)
        {
            NormalizedDiceAffectingItems.Add(Normalize(item));
        }
    }
    ...
    /// <summary>
    /// Checks if a single string token matches a registered dice-affecting item.
    /// </summary>
    public static bool IsDiceFaceAffecting(string token)
    {
        return NormalizedDiceAffectingItems.Contains(Normalize(token));
    }

    /// <summary>
    /// Scans a complex syntax expression (e.g. "x6.Basilisk Scale.part.0") to check if any constituent token matches.
    /// </summary>
    public static bool ExpressionContainsDiceAffectingItem(string expression)
    {
        if (string.IsNullOrWhiteSpace(expression)) return false;

        // Split by standard syntax delimiters to extract the item name safely
        string[] tokens = expression.Split(new[] { '.', '#', '(', ')', ' ' }, StringSplitOptions.RemoveEmptyEntries);
        foreach (var token in tokens)
        {
            if (IsDiceFaceAffecting(token))
            {
                return true;
            }
        }
...
public static class EffectKeywordColors
{
    ...
    public static readonly Dictionary<string, string> Descriptions = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        { "acidic", "+1 pip for each poison on me" },
        { "affected", "+1 pip for each 'thing' affecting me" },
        { "annul", "targets lose all keywords this turn" },
        { "antiDeathwish", "x2 except if I am dying this turn" },
        { "antiDog", "x2 except vs targets with equal hp to me" },
        { "antiEngage", "x2 except vs targets with full hp" },
        { "antiPair", "x2 except if this has the same pips as the previous dice this turn" },
        { "antiPristine", "x2 except if I have full hp" },
        { "armoured", "x2 if I have shields" },
        { "bloodlust", "bonus equal to the number of damaged enemies" },
        { "boned", "Summon a bones" },
        { "boost", "target gets +N to all sides this turn" },
        { "buffed", "+1 pip for each buff I have" },
        { "bully", "x2 if I have the most hp of all" },
        { "cantrip", "activates during rolling if it lands face-up" },
        { "century", "x2 vs targets with 100+ hp" },
        { "chain", "x2 if this shares a keyword with the previous dice this turn" },
        { "channel", "costs -1 mana each time it is cast (minimum 1)" },
        { "charged", "+1 pip for each stored mana" },
        { "cleanse", "reduce target's negative effects by N" },
        { "cleave", "also hits both sides of the target" },
        { "cooldown", "can only be cast once each turn" },
        { "copycat", "copy the keywords from the previous dice this turn" },
        { "critical", "50% chance for +1, rechecks each turn" },
        { "cruel", "x2 vs targets on half or less hp" },
        { "cruesh", "If cruel's condition is met, gain the effects of flesh" },
        { "damage", "damage the target for N" },
        { "death", "I die" },
        { "deathlust", "If deathwish's condition is met, gain the effects of bloodlust" },
        { "deathwish", "x2 if I am dying this turn" },
        { "decay", "gets -1 this fight after use" },
        { "defy", "+1 pip for each incoming damage" },
        { "dejavu", "copy the keywords from the sides I used last turn" },
        { "deplete", "costs +1 mana each time it is cast" },
        { "descend", "also hits below the target" },
        { "dispel", "Remove all traits from the target this fight" },
        { "dog", "x2 vs targets with equal hp to me" },
        { "dogma", "Only pips change when the side is replaced" },
        { "doubDiff", "doubled pip delta" },
        { "doublegrowth", "Like growth but activates twice" },
        { "doubled", "Other keywords see colored N as 2N" },
        { "doubleUse", "can be used twice in a turn" },
        { "duegue", "If duel's condition is met, gain the effects of plague" },
        { "duel", "x2 vs those targeting me this turn" },
        { "duplicate", "copy this onto all allied sides this turn" },
        { "echo", "copy the pips of previous dice this turn" },
        { "ego", "x2 vs myself" },
        { "eliminate", "target must have the least hp" },
        { "enduring", "keywords remain when the side is changed (if possible)" },
        { "engage", "x2 vs targets with full hp" },
        { "engarged", "If engage's condition is met, gain the effects of charged" },
        { "engine", "x4 if the conditions of both engage and pristine are met" },
        { "equipped", "+1 pip for each item I have equipped" },
        { "era", "+1 pip for each elapsed turn" },
        { "evil", "if this saves a hero, I die" },
        { "exert", "replace all sides with blanks (exerted) until the end of next turn" },
        { "fashionable", "bonus pips equal to the total tier of all items I have equipped" },
        { "fault", "Other keywords see colored N as -1" },
        { "fierce", "target flees if they have N or less hp" },
        { "first", "x2 if no dice have been used this turn" },
        { "fizz", "+1 pip for each ability used this turn" },
        { "flesh", "+1 pip for each hp I have" },
        { "fluctuate", "changes to a random side each turn, retaining keywords and pips" },
        { "flurry", "+1 pip for each time I have been used this turn" },
        { "focus", "x2 vs the target of the previous dice this turn" },
        { "fumble", "50% chance to be blank each turn" },
        { "future", "effect is delayed until the start of next turn" },
        { "generous", "cannot target myself" },
        { "groooooowth", "After use, all my sides get +1 this fight after use" },
        { "groupDecay", "The keyword 'decay' activates for all allies" },
        { "groupExert", "The keyword 'exert' activates for all allies" },
        { "groupGroooooowth", "The keyword 'groooooowth' activates for all allies" },
        { "groupGrowth", "The keyword 'growth' activates for all allies" },
        { "groupSingleUse", "The keyword 'singleUse' activates for all allies" },
        { "growth", "gets +1 this fight after use" },
        { "guilt", "if this is lethal, I die" },
        { "halveDeathwish", "x0.5 (rounded down) if I am dying this turn" },
        { "halveDuel", "x0.5 (rounded down) vs those targeting me this turn" },
        { "halveEngage", "x0.5 (rounded down) vs targets with full hp" },
        { "heal", "heal the target for N" },
        { "heavy", "target must have the most hp" },
        { "hoard", "+1 pip for each unequipped items" },
        { "hyena", "x2 vs targets with whom our hp is co-prime" },
        { "hyperBoned", "Summon N bones" },
        { "hyperGrowth", "gets +N this fight after use" },
        { "hyperUse", "can be used N times in a turn" },
        { "hypnotise", "set target's damage sides to 0" },
        { "inflictBoned", "add boned to target's sides for a turn" },
        { "inflictDeath", "add death to target's sides for a turn" },
        { "inflictExert", "add exert to target's sides for a turn" },
        { "inflictInflictDeath", "add inflictDeath to target's sides for a turn" },
        { "inflictInflictNothing", "add inflictNothing to target's sides for a turn" },
        { "inflictNothing", "add nothing to target's sides for a turn" },
        { "inflictPain", "add pain to target's sides for a turn" },
        { "inflictSelfShield", "add selfshield to target's sides for a turn" },
        { "inflictSingleUse", "add singleUse to target's sides for a turn" },
        { "inspired", "x2 if the previous dice this turn was higher" },
        { "lead", "After use, other hero's sides that share a type with this get +N this turn" },
        { "lucky", "pips are randomised to between 0 and current pips, changes each turn" },
        { "manacost", "costs N mana" },
        { "manaGain", "gain N mana" },
        { "mandatory", "must be used (if possible)" },
        { "minusEra", "-1 pip for each elapsed turn" },
        { "minusFlesh", "-1 pip for each hp I have" },
        { "moxie", "x2 if I have the least hp of all" },
        { "nothing", "this keyword has no effect" },
        { "onesie", "Other keywords see colored N as 1" },
        { "overdog", "x2 vs targets with less hp than me" },
        { "pain", "I take N damage" },
        { "pair", "x2 if this has the same value as the previous dice this turn" },
        { "patient", "x2 if I I was not used last turn" },
        { "paxin", "x3 if the conditions of pair xor chain is met" },
        { "permaBoost", "target gets +N to all sides this fight" },
        { "permissive", "any keyword can be added to this" },
        { "petrify", "Transforms N sides to stone this fight" },
        { "picky", "target must have exactly N hp" },
        { "plague", "+1 pip for each poison on all characters" },
        { "plus", "Other keywords see colored N as N+1" },
        { "poison", "also inflicts N unblockable damage at the end of each turn" },
        { "possessed", "targets as if used by the other side" },
        { "potion", "I discard my topmost 'potion' item permanently" },
        { "pristeel", "If pristine's condition is met, gain the effects of steel" },
        { "pristine", "x2 if I have full hp" },
        { "priswish", "x4 if the conditions of both pristine and deathwish are met" },
        { "quadUse", "can be used 4 times in a turn" },
        { "quin", "x5 if this has the same value as the 4 previous dice this turn" },
        { "rainbow", "+1 pip for each keyword on this side" },
        { "rampage", "can be reused if it was lethal" },
        { "ranged", "can target enemies in the back row and avoid on-hit passives" },
        { "reborn", "x2 if I died this fight" },
        { "regen", "heal for N at the end of each turn" },
        { "removed", "something has been removed, you probably shouldn't be seeing this..." },
        { "repel", "N damage to all enemies attacking the target" },
        { "rescue", "this side can be used again if it saves a hero" },
        { "resilient", "pips remain when the side is replaced" },
        { "resonate", "copy the effect of the previous dice this turn, retaining this side's pips and the resonate keyword" },
        { "revDiff", "inverted pip delta" },
        { "rite", "+1 for each unused ally, they become used" },
        { "run", "x3 if this forms a straight of length 3+ with the dice used before it this turn" },
        { "scared", "target must have N or less hp" },
        { "selfCleanse", "Like cleanse but hits me instead of the target" },
        { "selfHeal", "heal myself for N" },
        { "selfPetrify", "Like petrify but hits me instead of the target" },
        { "selfPoison", "Like poison but hits me instead of the target" },
        { "selfRegen", "Like regen but hits me instead of the target" },
        { "selfRepel", "Like repel but hits me instead of the target" },
        { "selfShield", "shield myself for N" },
        { "selfVulnerable", "Like vulnerable but hits me instead of the target" },
        { "sept", "x7 if this has the same value as the 6 previous dice this turn" },
        { "serrated", "x2 vs targets who have gained no shields this turn" },
        { "share", "targets gain all my keywords this turn (except share)" },
        { "shield", "shield the target for N" },
        { "shifter", "this side has a random extra keyword, changes each turn" },
        { "singleCast", "can only be cast once each fight" },
        { "singleUse", "after I use this side, replace it with a blank (used) this fight" },
        { "sixth", "x2 if this is the 6th dice you use this turn" },
        { "skill", "bonus pips equal to my level" },
        { "sloth", "x2 vs targets with fewer blank sides" },
        { "smith", "target gets +N to damage and shield sides this turn" },
        { "spellRescue", "cost is refunded if it saves a hero" },
        { "sprint", "x5 if this forms a straight of length 5+ with the dice used before it this turn" },
        { "spy", "copy all keywords from the first enemy attack" },
        { "squared", "Other keywords see colored N as N-squared" },
        { "squish", "x2 vs targets with the least hp of all" },
        { "stasis", "this side cannot change" },
        { "steel", "+1 pip for each shield I have" },
        { "step", "x2 if this forms a straight of length 2+ with the dice used before it this turn" },
        { "sticky", "cannot be rerolled" },
        { "swapCruel", "x2 if I have half or less hp" },
        { "swapDeathwish", "x2 if target is dying this turn" },
        { "swapEngage", "x2 if I have full hp" },
        { "swapTerminal", "x2 if I have 1 hp" },
        { "tactical", "counts twice for tactic costs" },
        { "tall", "x2 vs the topmost target" },
        { "terminal", "x2 vs targets on 1 hp" },
        { "threesy", "Other keywords see colored N as 3" },
        { "treble", "Other keywords x2 -> x3" },
        { "trill", "If trio's condition is met, gain the effects of skill" },
        { "trio", "x3 if this has the same value as the 2 previous dice this turn" },
        { "underdog", "x2 vs targets with more hp than me" },
        { "undergrowth", "After use, the opposite side gets +1 this fight after use" },
        { "underocus", "x4 if the conditions of both underdog and focus are met" },
        { "unusable", "cannot be used (manually)" },
        { "uppercut", "x2 vs targets with the most hp of all" },
        { "vigil", "+1 pip for each defeated ally" },
        { "vitality", "grants the target +N empty hp this fight" },
        { "vulnerable", "target takes +N damage from dice & abilities this turn" },
        { "weaken", "target gets -N to all sides this turn" },
        { "wham", "x2 vs targets with shields" },
        { "wither", "grants the target -N empty hp this fight" },
        { "zeroed", "Other keywords see colored N as 0" }
    };
