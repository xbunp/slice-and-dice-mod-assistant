...
public class FlexibleColorPicker : MonoBehaviour
{
    ...
    */
    ...
    [Tooltip("Texture to use as the cursor when picking a color from the screen. Optional.")]
    public Texture2D eyedropperCursor;
    [Tooltip("Hotspot for the eyedropper cursor (e.g., the tip of the dropper icon).")]
    public Vector2 eyedropperCursorHotspot = Vector2.zero;
    ...
    */
    ...
    {
        // Cancel eyedropper if window is closed mid-pick
        if (isPickingColor)
        {
    ...
    {
        if (isPickingColor) return; // Ignore standard drags if we are in eyedropper mode
    ...
    * ----------------------------------------------------------
    * 
    * Turns the cursor into a color picker to pick any color on
    * the screen (primarily UI and rendered pixels).
    */

    /// <summary>
    /// Starts the eyedropper screen-color picking mode.
    /// Link this to a UI Button's onClick event!
    /// </summary>
    public void StartEyedropper()
    {
        if (isPickingColor) return;
        StartCoroutine(EyedropperRoutine());
    }

    private IEnumerator EyedropperRoutine()
    {
        isPickingColor = true;

        // Change cursor to eyedropper texture
        if (eyedropperCursor != null)
        {
            Cursor.SetCursor(eyedropperCursor, eyedropperCursorHotspot, CursorMode.Auto);
        }

        // Wait one frame so we don't immediately capture the click on the button that started this routine
        yield return null;

        // Wait until user left-clicks or hits Escape to cancel
        ...
        // We must wait until the end of the frame to ensure all rendering (including UI) is complete
        yield return new WaitForEndOfFrame();

        Vector2 mousePos = Input.mousePosition;

        // Ensure we are inside the screen bounds
        if (mousePos.x >= 0 && mousePos.x < Screen.width && mousePos.y >= 0 && mousePos.y < Screen.height)
        {
            // Create a 1x1 texture to read the specific pixel
            Texture2D screenPixel = new Texture2D(1, 1, TextureFormat.RGB24, false);
            screenPixel.ReadPixels(new Rect(mousePos.x, mousePos.y, 1, 1), 0, 0);
            screenPixel.Apply();

            Color pickedColor = screenPixel.GetPixel(0, 0);
            Destroy(screenPixel);

            // Set picker color (preserve current alpha value since screen reading has no alpha)
            pickedColor.a = this.color.a;
